<?php
  ...
  if ( 'success' == $newsletter_status){
    ...
    $newsletter_message = sprintf('<h3 class="py-5 d-flex justify-content-center">%s</h3>',  pll__('Subscribed to the newsletter successfully.') );
  } elseif ( 'fail' === $newsletter_status){
    $newsletter_message = sprintf('<h3 class="pb-3 d-flex justify-content-center">%s</h3>', pll__('Subscription to the newsletter failed.') );
  }
      ...
      <div class="liana-form" id="newsletter">
        ...
        <form method="post" action="<?php the_sub_field("liana_subscription_page"); ?>" class="lianamailer">
          ...
          <div class="mb-3 email">
            ...
            <input id="newsletterEmail" name="email" type="email" aria-label="<?php pll_e('Leave an email address to subscribe to the newsletter'); ?>" class="form-control border border-dark" required></input>
          </div>
